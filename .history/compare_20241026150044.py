...
class TSPComparer:
        ...
        for start in self.cities:
            ...
            for perm in permutations(other_cities):
                ...
                for next_city in perm:
                    path_segment = nx.dijkstra_path(G, current_city, next_city, 
                                                  weight='weight')
                    segment_distance = nx.dijkstra_path_length(G, current_city, 
                                                             next_city, 
                                                             weight='weight')
                ...
                # Return to start
                final_segment = nx.dijkstra_path(G, current_city, start, 
                                               weight='weight')
                final_distance = nx.dijkstra_path_length(G, current_city, start, 
                                                       weight='weight')
    ...
    def solve_with_qaoa(self):
        ...
        """
        # Create cost Hamiltonian with stronger penalties
            ...
            for j in range(self.n_cities):
                if i != j:
                    for t in range(self.n_cities):
                        ...
                        cost_ops.append(PauliOp(pauli, self.distances[i][j] / 4))
        
        # Strong penalties for constraints
        penalty = 20.0  # Increased penalty
        ...
        for t in range(self.n_cities):
            for i, j in combinations(range(self.n_cities), 2):
                ...
                pauli_str[qubit1] = 'Z'
                pauli_str[qubit2] = 'Z'
                cost_ops.append(PauliOp(Pauli(''.join(pauli_str)), penalty))
        ...
        optimizer = COBYLA(maxiter=500)  # Increased iterations
        quantum_instance = QuantumInstance(
            Aer.get_backend('statevector_simulator'),
            shots=2048,  # Increased shots
            seed_simulator=123,
            seed_transpiler=123
        )
        ...
        qaoa = QAOA(
            ...
            reps=4,  # Increased QAOA layers
            initial_point=[np.pi/4] * 8  # Better initial point
        )
